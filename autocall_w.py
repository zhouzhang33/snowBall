...
class AutoCall():
    ...
    def spath_expect_payoff(self, spath):
        ...
        if len(ob) != 0:
            ...
            # payoff_ever_out = payoff_ever_out.sum()/simulations
            # ever_in = np.where(np.min(spath.loc[self.basic_parameter['observe_point_copy']].values, axis=0) <= ki)[0] #发生敲入的路径
            ...
            # payoff_only_in=np.exp(-r * T) * sum((list(map(lambda x: min(x,k),payoff_only_in)))/k*F-F)/simulations #只敲入相当于put，payoff最大为本金
            # noi = list((set(path_col) ^ set(ever_out) ^ set(only_in))&set(ever_nlizard)) #未敲入或者敲出
            # payoff_noi = len(noi) * (dividend*T) * F * np.exp(-r * T)/simulations
